...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/campaigns/:id/milestones/:milestoneId/complete", async (req: Request, res: Response) => {
    try {
      const { evidence } = req.body;
      
      if (!evidence) {
        return res.status(400).json({ message: "Evidence is required to complete milestone" });
      }
      
      await storage.completeMilestone(
  ...
  app.post("/api/scraper/start", async (req: Request, res: Response) => {
    try {
      ...
      const { campaignScheduler } = await import('./services/scheduler');
      campaignScheduler.start(intervalMinutes);
      
      res.json({ 
        success: true, 
        message: `Scraper started with ${intervalMinutes} minute intervals`,
  ...
  app.post("/api/scraper/stop", async (req: Request, res: Response) => {
    try {
      const { campaignScheduler } = await import('./services/scheduler');
  ...
  app.post("/api/scraper/monitor/:campaignId", async (req: Request, res: Response) => {
    try {
      const campaignId = parseInt(req.params.campaignId);
      
      const { campaignScheduler } = await import('./services/scheduler');
      await campaignScheduler.monitorSpecificCampaign(campaignId);
      
      res.json({ 
        success: true, 
        message: `Campaign ${campaignId} monitoring completed`
      });
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      res.status(500).json({ error: message });
  ...
  app.get("/api/campaigns/:id/milestones/:milestoneId/evidence", async (req: Request, res: Response) => {
    try {
      const campaignId = parseInt(req.params.id);
      const milestoneId = req.params.milestoneId;
      
      const { campaignScraper } = await import('./services/scraper');
      const evidence = await campaignScraper.getScrapedEvidence(milestoneId, campaignId);
