...
const CampaignDetails = () => {
    ...
    try {
      ...
      if (backendCampaign) {
        const formattedCampaign: CampaignMetadata = {
          ...
          milestones: Array.isArray(backendCampaign.milestones) ? backendCampaign.milestones : (backendCampaign.milestones ? JSON.parse(backendCampaign.milestones) : []),
  ...
  const handleMilestoneComplete = async (milestoneId: string, evidence: string) => {
    if (!campaign) return;
    
    try {
      await apiRequest(`/api/campaigns/${campaign.pId}/milestones/${milestoneId}/complete`, 'POST', {
        evidence
      });
      
      toast({
        title: 'Milestone Completed!',
        description: 'Funds have been released for this milestone.',
      });
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl overflow-hidden shadow-campaign mb-8">
            ...
            <div className="p-6">
              ...
              {campaign.milestones && campaign.milestones.length > 0 && (
                <div className="mb-8">
                  <h2 className="text-xl font-bold mb-4">Fund Release Milestones</h2>
                  <MilestoneDisplay
                    milestones={campaign.milestones}
                    amountCollected={campaign.amountCollected}
                    target={campaign.target}
                    campaignId={campaign.pId}
                    isCreator={campaign.owner.toLowerCase() === address?.toLowerCase()}
                    onMarkComplete={handleMilestoneComplete}
                  />
                </div>
              )}
